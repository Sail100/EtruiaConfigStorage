...
-- THIS SCRIPT WILL BE FOR DEVELOPMENT AND OTHER PURPOSES.  EXPECT THIS NOT TO WORK SOMETIMES.
local OrionLib = loadstring(game:HttpGet(('https://raw.githubusercontent.com/shlexware/Orion/main/source')))()
local Window = OrionLib:MakeWindow({Name = "Etruia Development Installtion UI", HidePremium = false, IntroText = "Etruia installion UI is loading.."})
local lplr = game.Players.LocalPlayer
...
function installv2()
    ...
    task.spawn(function()
        if readfile('vape/Profiles/Render_Blurple6872274481.vapeprofile.txt') then
            if readfile('vape/Profiles/6872265039Render_BlurpleGUIPositions.vapeprofile.txt') then
                print("Profile(s) installed, continuing installiton")
                task.wait(1)
                end
            end
        elseif not readfile('vape/Profiles/Render_Blurple6872274481.vapeprofile.txt') then
            if not readfile('vape/Profiles/6872265039Render_BlurpleGUIPositions.vapeprofile.txt') then
                lplr:Kick("womp womp, make sure you INSTALL profiles.")
                end
            end
    end)
