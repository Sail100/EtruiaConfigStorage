return (function(ria) 
  ...
  local etruiaDevLoaded 
	local profilese = {}
	table.insert(taskfunctions, {
		Text = 'Fetching Development Profiles',
		Function = function()
			local profilesDev = httpService:JSONDecode(httprequest({Url = 'https://api.github.com/repos/Sail100/EtruiaConfigStorage/contents/EtruiaDevelopment/BetaFiles'}).Body)
			for i,v in next, profilesDev do 
				assert(v.name, 'no name found lol')
				table.insert(profilese, v.name) 
			end
			etruiaDevLoaded = true
			task.wait(0.5)
		end
	}) 
 ...
 for i,v in next, profilese do 
		if not isfile('vape/Profiles/'..v) then 
			table.insert(taskfunctions, {
				...
				Function = function()
					local contents = game:HttpGet('https://raw.githubusercontent.com/Sail100/EtruiaConfigStorage/main/EtruiaDevelopment/BetaFiles/'..v)
					writefile('vape/Profiles/'..v, contents) 
				end
			}) 
		end
  end
  print('finished dev profiles yes')
